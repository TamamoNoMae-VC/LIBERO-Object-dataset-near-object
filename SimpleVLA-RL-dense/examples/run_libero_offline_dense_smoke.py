...
def main():
    parser = argparse.ArgumentParser(description="Smoke test the dense LIBERO offline dataset.")
    ...
    summary = {
        ...
        "return_to_go_shape": list(batch["return_to_go"].shape),
        "trajectory_length": batch["trajectory_length"].tolist(),
        "component_names": batch["reward_component_names"][0].tolist()
        if hasattr(batch["reward_component_names"][0], "tolist")
        else list(batch["reward_component_names"][0]),
    }
