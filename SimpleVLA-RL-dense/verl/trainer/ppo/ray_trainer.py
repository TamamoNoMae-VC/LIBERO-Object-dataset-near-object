...
class RayTrainer(object):
    ...
    def _create_dataloader(self):   # next fix
        ...
        from verl.utils.dataset import LiberoOfflineInitDataset
        ...
        self.offline_init_dataloader = None
        self.offline_init_remaining_steps = int(self.config.data.get('offline_init_steps', 0) or 0)
        if self.config.data.get('offline_init_manifest'):
            offline_batch_size = self.config.data.get('offline_init_batch_size') or self.config.data.train_batch_size
            self.offline_init_dataset = LiberoOfflineInitDataset(
                manifest_path=self.config.data.offline_init_manifest,
                sample_num=self.config.data.get('offline_init_sample_num', -1),
            )
            self.offline_init_dataloader = DataLoader(
                dataset=self.offline_init_dataset,
                batch_size=offline_batch_size,
                shuffle=False,
                drop_last=False,
                collate_fn=collate_fn,
            )
            self.offline_init_iter = iter(self.offline_init_dataloader)
        ...
        for epoch in range(self.config.trainer.total_epochs):
            ...
            while True:
                ...
                while len(valid_batch) < batch_size * n_samples:
                    try:
                        using_offline_init = self.offline_init_dataloader is not None and self.offline_init_remaining_steps > 0
                        if using_offline_init:
                            try:
                                batch_dict = next(self.offline_init_iter)
                            except StopIteration:
                                self.offline_init_iter = iter(self.offline_init_dataloader)
                                batch_dict = next(self.offline_init_iter)
                            self.offline_init_remaining_steps -= 1
                        else:
                    ...
                    with Timer(name='gen', text="{name}: {seconds:.1f} seconds") as timer:
                        ...
                        non_tensor_keys = {"task_suite_name"}
                        for extra_key in (
                            "custom_object_position",
                            "custom_object_quat",
                            "custom_target_position",
                            "custom_reference_position",
                            "movable_object_name",
                            "reference_object_name",
                            "offline_source_episode",
                            "data_source",
                        ):
                            if extra_key in newbatch.non_tensor_batch:
                                non_tensor_keys.add(extra_key)
                        if "robotwin" in self.config.data.task_suite_name:
                            ...
                            if 'trial_seed' in newbatch.batch:
                                batch_keys.append('trial_seed')
                            gen_batch = newbatch.select(
                        ...
                        gen_batch.meta_info = {
                            ...
                            'n_samples': n_samples,
                            'pad_token_id': self.tokenizer.pad_token_id,
                            'offline_init': using_offline_init,
                        }
