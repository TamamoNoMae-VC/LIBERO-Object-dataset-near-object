...
def _candidate_names(names, needle):
...
def _resolve_first_name(names, aliases, label):
    candidates = []
    for alias in aliases:
        candidates.extend(_candidate_names(names, alias))
    candidates = list(dict.fromkeys(candidates))
    if not candidates:
        raise RuntimeError(f"Could not resolve {label} from aliases={aliases!r}")
    return candidates[0]


def _apply_custom_libero_init(env, custom_init):
    if not custom_init:
        return
    ...
    env.sim.data.set_joint_qpos(
        joint_name,
        np.concatenate([object_position, object_quat]).astype(np.float64),
    )
    try:
...
def _get_body_position(env, body_name):
    body_id = env.sim.model.body_name2id(body_name)
    return np.asarray(env.sim.data.body_xpos[body_id], dtype=np.float64).copy()


def _within_custom_camera_bounds(config, position):
    return bool(
        config.camera_x_min <= float(position[0]) <= config.camera_x_max
        and config.camera_y_min <= float(position[1]) <= config.camera_y_max
    )
...
def _compute_custom_libero_success(env, config, custom_init):
    ...
    reference_position = _get_body_position(env, reference_name)
    target_position = np.asarray(custom_init.get("custom_target_position"), dtype=np.float64) if custom_init and custom_init.get("custom_target_position") is not None else None
    reference_distance = float(np.linalg.norm(object_position[:2] - reference_position[:2]))
    outside_basket = reference_distance >= float(config.basket_exclusion_radius)
    near_basket = float(config.near_basket_distance_min) <= reference_distance <= float(config.near_basket_distance_max)
    visible = _within_custom_camera_bounds(config, object_position)
    z_close = abs(float(object_position[2] - reference_position[2])) <= float(config.custom_target_height_tol)
    target_close = True
    if target_position is not None:
        target_close = float(np.linalg.norm(object_position[:2] - target_position[:2])) <= float(config.custom_target_tolerance_xy)
        z_close = abs(float(object_position[2] - target_position[2])) <= float(config.custom_target_height_tol)
    success = bool(outside_basket and near_basket and visible and z_close and target_close)
    return success, {
        "object_position": object_position.tolist(),
        "reference_position": reference_position.tolist(),
        "reference_distance_xy": reference_distance,
        "visible_in_camera": visible,
        "target_position": target_position.tolist() if target_position is not None else None,
        "target_close": target_close,
    }

def env_worker(task_name, task_id, trial_id, config, input_queue, output_queue, is_valid, global_steps, max_steps, custom_init=None):
    ...
    _apply_custom_libero_init(env, custom_init)
    custom_task_name = str(getattr(config, "custom_task_name", task_name))
    ...
    output_queue.put({
        ...
        'task_file_name': f"{custom_task_name}_task_{task_id}_trial_{trial_id}",
        'active': True,
        'complete': False,
        'finish_step': 0,
        'custom_success_debug': None,
    })
    ...
    while True:
        ...
        for i in range(len(action)):
            ...
            complete, custom_debug = _compute_custom_libero_success(env, config, custom_init or {})
            ...
            finish_step += 1
            if done or complete or finish_step >= max_steps:
                active = False
                break
        ...
        output_data = {
            ...
            'custom_success_debug': custom_debug,
        }
...
class RobHFRollout(BaseRollout):
    ...
    def _generate_minibatch_libero(self, prompts):
        ...
        task_id = prompts.batch['task_id'].repeat_interleave(n_samples, dim=0)
        trial_id = prompts.batch['trial_id'].repeat_interleave(n_samples, dim=0)
        trial_seed = prompts.batch['trial_seed'].repeat_interleave(n_samples, dim=0) if 'trial_seed' in prompts.batch else None
        task_suite_name = np.repeat(prompts.non_tensor_batch['task_suite_name'], n_samples)
        custom_object_position = (
            np.repeat(prompts.non_tensor_batch['custom_object_position'], n_samples, axis=0)
            if 'custom_object_position' in prompts.non_tensor_batch else None
        )
        custom_object_quat = (
            np.repeat(prompts.non_tensor_batch['custom_object_quat'], n_samples, axis=0)
            if 'custom_object_quat' in prompts.non_tensor_batch else None
        )
        movable_object_name = (
            np.repeat(prompts.non_tensor_batch['movable_object_name'], n_samples)
            if 'movable_object_name' in prompts.non_tensor_batch else None
        )
        custom_target_position = (
            np.repeat(prompts.non_tensor_batch['custom_target_position'], n_samples, axis=0)
            if 'custom_target_position' in prompts.non_tensor_batch else None
        )
        ...
        for idx in range(batch_size):
            ...
            tr_id = trial_id[idx][0].item()
            custom_init = None
            if custom_object_position is not None and custom_object_quat is not None:
                custom_init = {
            ...
            p = Process(
                target=env_worker,
                args=(task_name, t_id, tr_id, self.config, input_q, output_q, is_valid, global_steps, max_steps, custom_init)
            )
            p.start()
            processes.append(p)
